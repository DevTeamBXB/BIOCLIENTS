...
export default function OrderForm({
  ...
  const [products, setProducts] = useState<SelectedProduct[]>([
    { _id: '', cantidadVacios: 0, cantidadLlenos: 0, quantity: 1 }
  ]);
...
const updateProduct = (
        ...
        ? {
            ...
            [field]:
              field === '_id'
                ? String(value)
                : field === 'quantity'
                ? Math.max(1, Number(value)) // ✅ nunca deja quantity en 0
                : Number(value) || 0, // vacíos/llenos sí pueden ser 0
          }
        : p
    )
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validación: todos los productos deben tener _id y quantity >= 1
    if (products.some((p) => !p._id || (p.quantity || 0) < 1)) {
      return alert('Cada producto debe estar seleccionado y tener cantidad mínima de 1');
    }
    ...
    try {
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          observaciones,
          products,
          classification // ✅ enviamos "Equipos Biomedicos"
        })
      });
      ...
      {products.map((product, index) => (
        ...
        >
          ...
          >
            ...
            {availableProducts
              .filter((p) => p.businessLine === 'Equipos Biomedicos')
              .map((p) => (
